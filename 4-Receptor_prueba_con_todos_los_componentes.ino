...
TimeSeries ts1(6, "temperature_degrees", "{job=\"Fire-stop\",host=\"esp32\",device=\"esp32-rep-siria\"}");

TimeSeries ts2(6, "humidity", "{job=\"Fire-stop\",host=\"esp32\",device=\"esp32-rep-siria\"}");

TimeSeries ts3(6, "velocity", "{job=\"Fire-stop\",host=\"esp32\",device=\"esp32-rep-siria\"}");

TimeSeries ts4(6, "position", "{job=\"Fire-stop\",host=\"esp32\",device=\"esp32-rep-siria\"}");

TimeSeries ts5(6, "monoxide", "{job=\"Fire-stop\",host=\"esp32\",device=\"esp32-rep-siria\"}");

TimeSeries ts6(6, "battery", "{job=\"Fire-stop\",host=\"esp32\",device=\"esp32-rep-siria\"}");
...
char resp [8]={0};
int numero=0;
int device, temperature, humidity, velocity, pos, CO2;
int diox;
int battery = 0;
int loopCounter = 0;
...
void loop() 
{
  ...
  if (e32ttl.available()  > 1){
    ...
    //Changing diox variable to the opposite value
    
    ts1.addSample(time, temperature);
    ts2.addSample(time, humidity);
    ts3.addSample(time, velocity);
    ts4.addSample(time, pos);
    ts5.addSample(time, diox);
    ts6.addSample(time, battery);
